...
if nargout>1
  ...
  for i=1:N9
    for j=1:N3 
      if ( (i/N9)^2 + (j/N3)^2 < 1 ), T(i,j) = sE1; end 
    end
  end
  T = [fliplr(T),T]; 
  T = [flipud(T);T]; 
  x(2+(1:2*N9),N3-1+(1:2*N3)) = T; 
 
  % Add a smaller ellipse. 
  T = zeros(N9,N3); 
  for i=1:N9
    for j=1:N3 
      if ( (i/N9)^2 + (j/N3)^2 < 0.6 ), T(i,j) = sE1; end 
    end
  end
  T = [fliplr(T),T];
  T = [flipud(T);T];
  x(N9+(1:2*N9),N3-1+(1:2*N3)) = x(N9+(1:2*N9),N3-1+(1:2*N3)) + 2*T;
  % Correct for overlap.
  f = find(x==3);
  x(f) = 2*ones(size(f));
  ...
  % Add a cross.
  T = zeros(2*N9+1,2*N9+1);
  [mT,nT] = size(T);
  T(N9+1,1:nT) = ones(1,nT);
  T(1:mT,N9+1) = ones(mT,1);
  x(N2+N12+(1:mT),N2+(1:nT)) = 5*T;
